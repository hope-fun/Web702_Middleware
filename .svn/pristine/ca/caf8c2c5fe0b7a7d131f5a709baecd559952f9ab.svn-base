...
import net.luckywings.mobigame.server.persistence.service.ISimpleStaticDAO;
...
public abstract class AbstractSimpleStaticDAO<T extends AbstractStaticModel> implements
        ISimpleStaticDAO<T> {
    ...
    private final Logger log = Logger.getLogger(AbstractSimpleStaticDAO.class);


    public AbstractSimpleStaticDAO() {}
    ...
    @SuppressWarnings("unchecked")
    @Transactional(readOnly = true)
    public List<T> findByType(EnumFieldPjz type) {
        Query query =
                this.entityManager.createQuery("from " + getDomainClassString()
                        + " where type=:type");
    ...
    @Transactional(readOnly = true)
    protected <T> IPagedList<T> getPagedList(TypedQuery<Long> totalQuery, TypedQuery<T> query,
            int index, int size) {
    ...
    @Override
    public void deletePhysically(final long entityId) {
        final T entity = this.findOne(entityId);
        if (entity != null) {
            this.entityManager.remove(entity);
        }
    ...
    protected String getDomainClassString() {
        ...
        return MODEL_PACKAGE_NAME + "."
                + className.substring(0, className.indexOf(SERVICE_CLASS_SUFFIX));
    }
