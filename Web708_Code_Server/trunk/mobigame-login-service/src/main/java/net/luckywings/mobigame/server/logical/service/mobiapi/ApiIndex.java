...
public class ApiIndex {
	...
	// 不需要登录的模块列表
	private static final String accessList = "server/list, user/register, user/login";
	...
	@POST
	@Path("/{v}/{m}/{a}")
	public Response dispatch(@PathParam("v") int v, @PathParam("m") String m, @PathParam("a") String a, @RequestBody String requestString) {
...
//				} else { // 正常处理请求
			        requestObj.setIp(request.getRemoteAddr());
					ApiBaseResponse apiResponse = (ApiBaseResponse) module.execute(method, requestObj);
					sid = module.getSessionId();
					builder = apiResponse.getResponseBuilder();
//				}
			}
